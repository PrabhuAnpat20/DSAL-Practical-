...
public:
  ...
  void calculate_wt(double *p, double *q, int n, int st[]) {
  ...
  void show(double **w, double **r, double **c, int n) {
    for (int i = 0; i < n; i++) {
      for (int j = i; j < n; j++) {
        cout << w[i][j] << "  " << r[i][j] << "  " << c[i][j] << " | ";
      }

      cout << endl;
    }
    cout<<"cost="<<c[0][4]<<endl;
  }

  node *createTree(int i, int j, double **r, int *st) {
    if (i != j) {
      int k = r[i][j];
